...
function lazyExport$$module$tmp$serialize(callback, self, key, index_doc, index, data) {
  return $jscomp.asyncExecutePromiseGeneratorFunction(function*() {
    const res = callback(key, JSON.stringify(data));
    if (res && res["then"]) {
      yield res;
    }
    return yield self.export(callback, self, key, index_doc, index + 1);
  });
}
function exportIndex$$module$tmp$serialize(callback, self, field, index_doc, index) {
  ...
  return $jscomp.asyncExecutePromiseGeneratorFunction(function*() {
    ...
    switch(index || (index = 0)) {
      case 0:
        key = "reg";
        if ($jscomp$async$this.fastupdate) {
          data = create_object$$module$tmp$common();
          for (let key in $jscomp$async$this.register) {
            data[key] = 1;
          }
        } else {
          data = $jscomp$async$this.register;
        }
        break;
      case 1:
        key = "cfg";
        data = {"doc":0, "opt":$jscomp$async$this.optimize ? 1 : 0};
        break;
      case 2:
        key = "map";
        data = $jscomp$async$this.map;
        break;
      case 3:
        key = "ctx";
        data = $jscomp$async$this.ctx;
        break;
      default:
        return true;
    }
    return yield lazyExport$$module$tmp$serialize(callback, self || $jscomp$async$this, field ? field + "." + key : key, index_doc, index, data);
  });
}
...
function exportDocument$$module$tmp$serialize(callback, self, field, index_doc, index) {
  ...
  return $jscomp.asyncExecutePromiseGeneratorFunction(function*() {
    ...
    if (index_doc < $jscomp$async$this.field.length) {
      const field = $jscomp$async$this.field[index_doc];
      const idx = $jscomp$async$this.index[field];
      self = $jscomp$async$this;
      if (!(yield idx.export(callback, self, index ? field.replace(":", "-") : "", index_doc, index++))) {
        index_doc++;
        index = 1;
        yield self.export(callback, self, field, index_doc, index);
      }
      return true;
    } else {
      let key, data;
      switch(index) {
        case 1:
          key = "tag";
          data = $jscomp$async$this.tagindex;
          break;
        case 2:
          key = "store";
          data = $jscomp$async$this.store;
          break;
        default:
          return true;
      }
      return yield lazyExport$$module$tmp$serialize(callback, $jscomp$async$this, key, index_doc, index, data);
    }
  });
}
...
Index$$module$tmp$index.prototype.search = function(query, limit, options) {
  if (!options) {
    if (!limit && is_object$$module$tmp$common(query)) {
      ...
      if (is_object$$module$tmp$common(limit)) {
        options = limit;
      }
    }
  }
...
if ((tmp$$module$tmp$webpack = root$$module$tmp$webpack["define"]) && tmp$$module$tmp$webpack["amd"]) {
  ...
  if (root$$module$tmp$webpack["exports"]) {
